<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <span class="status status-lg status-{{ $booking->status->value }}">{{ ucfirst($booking->status->value) }}</span>
        </div>
    </x-slot>
    ...
    <div class="booking-detail-grid">
        <div>
            <section class="panel">
                @php
                    $steps = ['pending' => 1, 'confirmed' => 2, 'completed' => 3];
                    $progress = $steps[$booking->status->value] ?? ($booking->status->value === 'cancelled' || $booking->status->value === 'rejected' ? 1 : 1);
                @endphp
                <div class="booking-progress">
                    @foreach ([['Request sent','Owner review'],['Confirmed','Court reserved'],['Completed','Ready to review']] as $index => [$label,$copy])
            ...
            @if ($booking->status->value === 'completed' && ! $booking->review)
                <section class="panel mt-6">
                    ...
                    <form method="POST" action="{{ route('reviews.store', $booking) }}" class="mt-5 space-y-4">
                        @csrf
                        <div><label>Rating</label><select class="form-input" name="rating" required><option value="">Choose</option>@for($i=5;$i>=1;$i--)<option value="{{ $i }}">{{ $i }} star{{ $i > 1 ? 's' : '' }}</option>@endfor</select></div>
                        <div><label>Review</label><textarea class="form-input min-h-28" name="body" required minlength="10" maxlength="1500"></textarea></div>
                        <button class="btn-primary">Publish verified review</button>
                    </form>
                </section>
            @endif
        </div>
        ...
        <aside>
            <section class="panel">
                <div class="panel-heading"><div><p class="eyebrow">Payment tracking</p><h2>{{ ucfirst($booking->payment_status->value) }}</h2></div></div>
                ...
                @if (in_array($booking->status->value, ['pending','confirmed']) && $booking->court->payment_policy !== 'pay_on_site')
                    <form method="POST" action="{{ route('payments.store', $booking) }}" enctype="multipart/form-data" class="mt-5 space-y-4">
                        @csrf
                        <div>
                            <label>Payment method</label>
                            <select class="form-input" name="court_payment_method_id">
                        ...
                        <div><label>Amount sent</label><input class="form-input" type="number" min="0.01" step="0.01" name="amount" value="{{ number_format($booking->price_centavos / 100, 2, '.', '') }}" required></div>
                        <div><label>Transaction reference</label><input class="form-input" name="reference" maxlength="120"></div>
                        <div><label>Proof <span>(JPG, PNG, WebP, or PDF)</span></label><input class="form-input" type="file" name="proof" accept=".jpg,.jpeg,.png,.webp,.pdf"></div>
