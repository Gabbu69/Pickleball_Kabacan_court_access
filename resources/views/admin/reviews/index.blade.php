    ...
    <div class="space-y-5">
        @forelse ($reviews as $review)
            <article class="panel flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                ...
                <form method="POST" action="{{ route('admin.reviews.update', $review) }}" class="flex gap-2">@csrf @method('PATCH')<select class="form-input" name="status"><option value="published" @selected($review->status === 'published')>Published</option><option value="hidden" @selected($review->status === 'hidden')>Hidden</option></select><button class="btn-outline">Update</button></form>
            </article>
        @empty
